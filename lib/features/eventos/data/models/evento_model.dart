...
class EventoModel extends Evento {
  ...
  factory EventoModel.fromJson(Map<String, dynamic> json) {
    return EventoModel(
      ...
      descripcion: _parseOptionalString(
        _readValue(json, 'descripcion'),
      ),
      fechaInicio: _parseRequiredDateTime(
        _readValue(
          json,
          'fechaInicio',
          alternativeKey: 'fecha_inicio',
        ),
      ...
      categoriaNombre: _parseOptionalString(
        _readValue(
          json,
          'categoriaNombre',
          alternativeKey: 'categoria_nombre',
        ),
      ),
      municipio: _parseOptionalString(
        _readValue(json, 'municipio'),
      ),
      activo: _parseBool(
        _readValue(json, 'activo'),
      ),
      fechaCreacion: _parseRequiredDateTime(
        _readValue(
          json,
          'fechaCreacion',
          alternativeKey: 'fecha_creacion',
        ),
  ...
  static dynamic _readValue(
    ...
    if (alternativeKey != null &&
        json.containsKey(alternativeKey)) {
  ...
  static String _parseRequiredString(
    ...
    if (parsedValue == null || parsedValue.isEmpty) {
      throw FormatException(
        'El campo obligatorio "$fieldName" no es válido',
      );
    }
  ...
  static DateTime _parseRequiredDateTime(
    ...
    if (parsedValue == null) {
      throw FormatException(
        'La fecha obligatoria "$fieldName" no es válida',
      );
    }
